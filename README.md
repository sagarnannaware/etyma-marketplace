...
A shared entry is signed by its author: every file hashed, the author's account claimed, and an Ed25519 signature over the lot, in `meta/<slug>.sig.json`. The private half of the key never leaves the machine that made it.

`verify-against-core.mjs` checks it here, with Etyma's own verifier rather than a copy, and `etyma verify <dir>` checks it anywhere. A signature that does not check out fails the build; a **missing** one does not, because the curated libraries in this repository are generated by CI and have no author to sign them.

What a signature means, exactly: **these are the author's bytes, unchanged**. It does not mean the library is safe, and nothing here treats it as though it did — that is what the rules below and your own reading are for.
