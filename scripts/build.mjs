...
// ─── Themes: a look as data ──────────────────────────────────────────────────
//
// `themes/<slug>.json` is an entry of kind `theme`: the colour tokens (light,
// and dark when authored), and optionally the style (type and density) and
// the typography the style needs. Nothing installs — the IDE's Look pane and
// the Assistant's `set_theme` APPLY it to the solution's theme — so a theme has
// no folder, no bundle and no module id. It is listed in the same catalogue so
// "a warm serif look" is one search away, like "add business days".
const THEMES = join(ROOT, "themes");
const themes = [];
if (existsSync(THEMES)) {
  for (const f of readdirSync(THEMES).filter((x) => x.endsWith(".json")).sort()) {
    const slug = f.slice(0, -5);
    const t = JSON.parse(readFileSync(join(THEMES, f), "utf8"));
    if (t.slug && t.slug !== slug) { console.error(`themes/${f}: slug "${t.slug}" does not match the file name`); process.exit(1); }
    if (!t.tokens || !t.tokens.light) { console.error(`themes/${f}: no tokens.light — a theme is its colours first`); process.exit(1); }
    themes.push({
      slug, kind: "theme",
      name: t.name || slug, version: t.version || "1.0.0",
      tagline: t.tagline || "", description: t.description || "", category: t.category || "Themes", tags: t.tags || [],
      source: t.source === "platform" ? "platform" : "curated",
      ...(t.author ? { author: t.author } : {}), ...(t.license ? { license: t.license } : {}),
      functions: [], components: [], packages: [],
      theme: { tokens: t.tokens, ...(t.style ? { style: t.style } : {}), ...(t.typography ? { typography: t.typography } : {}) },
    });
  }
}
...
const catalog = {
  ...
  generatedFrom: "libraries-src/*.lib.mjs, themes/*.json",
  count: built.length + contributed.length + themes.length,
  libraries: [
...
if (CHECK) {
  ...
  console.log(
    `built ${built.length} curated + ${contributed.length} contributed = ${built.length + contributed.length} libraries — ` +
      `${fns} functions, ${cmps} components${themes.length ? `; ${themes.length} theme${themes.length === 1 ? "" : "s"}` : ""} → libraries/, catalog.json`,
  );
}
